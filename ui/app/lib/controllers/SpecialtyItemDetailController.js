...
export default class SpecialtyItemDetailController extends DetailController {
  /*@ngInject*/
  constructor($mdDialog, $scope, $routeParams, SpecialtyItem) {
    super()
    if ($routeParams.specialtyItemId) {
      ...
      {
      if (item.label) {
        SpecialtyItem.update(item).then(function (data) { $mdDialog
          .show($mdDialog
          .alert()
          .title('Changes Saved!')
      ...
      $scope.create = item =>
        SpecialtyItem.create(item).then(
          data =>
            $mdDialog.show(
              $mdDialog.alert()
