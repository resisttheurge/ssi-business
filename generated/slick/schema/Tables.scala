...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription = Array(Addresses.schema, Carriers.schema, Contacts.schema, Customers.schema, Drawings.schema, JobAddresses.schema, Jobs.schema, JobSystemTypes.schema, Manufacturers.schema, Marks.schema, PartOrders.schema, Salespeople.schema, Schedules.schema, ShipmentItems.schema, Shipments.schema, ShippingGroupItems.schema, ShippingGroups.schema, ShippingItems.schema, ShippingItemZones.schema, ShippingRequests.schema, Shops.schema, SpecialtyItems.schema, SystemTypes.schema, UserRoles.schema, Users.schema, Vendors.schema, Zones.schema).reduceLeft(_ ++ _)
  ...
  /** Entity class storing rows of table PartOrders
    ...
    * @param abmNumber         Database column abm_number
    * @param status            Database column status SqlType(ENUM), Length(9,false)
    * @param partType          Database column part_type
    * @param partNumber        Database column part_number
    * @param partDescription   Database column part_description
  ...
  implicit def GetResultPartOrdersRow(implicit e0: GR[Int], e1: GR[Option[Int]], e2: GR[String], e3: GR[Option[String]], e4: GR[Option[java.sql.Date]]): GR[PartOrdersRow] = GR {
    prs => import prs._
      PartOrdersRow.tupled((<<[Int], <<[Int], <<?[Int], <<?[Int], <<[String], <<?[String], <<?[String], <<?[String], <<?[Int], <<?[Int], <<?[String], <<[Int], <<[Int], <<[Int], <<?[java.sql.Date], <<?[java.sql.Date], <<?[java.sql.Date], <<?[String]))
  }
  ...
  class PartOrders(_tableTag: Tag) extends Table[PartOrdersRow](_tableTag, "part_orders") {
    def * = (id, jobId, drawingId, abmNumber, status, partType, partNumber, partDescription, manufacturerId, vendorId, po, requestedQuantity, stockQuantity, purchaseQuantity, requestDate, purchaseDate, releaseDate, releasedBy) <>(PartOrdersRow.tupled, PartOrdersRow.unapply)

    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(jobId), drawingId, abmNumber, Rep.Some(status), partType, partNumber, partDescription, manufacturerId, vendorId, po, Rep.Some(requestedQuantity), Rep.Some(stockQuantity), Rep.Some(purchaseQuantity), requestDate, purchaseDate, releaseDate, releasedBy).shaped.<>({ r => import r._; _1.map(_ => PartOrdersRow.tupled((_1.get, _2.get, _3, _4, _5.get, _6, _7, _8, _9, _10, _11, _12.get, _13.get, _14.get, _15, _16, _17, _18))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
    ...
    val abmNumber: Rep[Option[Int]] = column[Option[Int]]("abm_number", O.Default(None))
    /** Database column status SqlType(ENUM), Length(9,false) */
    val status: Rep[String] = column[String]("status", O.Length(9, varying = false))
    val partType: Rep[Option[String]] = column[Option[String]]("part_type", O.Length(4, varying = false), O.Default(None))
    /** Database column part_number, Default(None) */
    val partNumber: Rep[Option[String]] = column[Option[String]]("part_number", O.Default(None))
    val partDescription: Rep[Option[String]] = column[Option[String]]("part_description", O.Default(None))
